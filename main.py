...
from actions.startup_routine   import startup_routine, run_startup_phase3
...
TOOL_DECLARATIONS = [
    ...
    },
    {
        ...
        "description": (
            "Sets, updates, shows, disables, enables, or runs the user's startup phase 3 routine. "
            "Use this when the user says things like 'set my startup routine to open Chrome and Discord', "
            "'on startup open ChatGPT', 'make phase 3 open these apps', or asks about the startup routine. "
            "This routine runs automatically after morning briefing and news on future launches."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "action": {
                    "type": "STRING",
                    "description": "set | update | show | status | disable | enable | run"
                },
                "instruction": {
                    "type": "STRING",
                    "description": "Natural language routine, e.g. 'open Chrome to ChatGPT and open Discord app'"
                }
            },
            "required": ["action"]
        }
    ...
    },
    {
        "name": "send_message",
        "description": (
            "Sends TEXT MESSAGES ONLY via WhatsApp, Telegram, or another messaging platform. "
            "Never use this for calls, voice calls, audio calls, video calls, ringing someone, "
            "or opening a call. For WhatsApp calls use the whatsapp plugin with action='audio_call'. "
            "For Discord calls use discord action='audio_call'. For Instagram calls use instagram action='audio_call'."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
...
class JarvisLive:
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        try:
            ...
            elif name == "startup_routine":
                r = await loop.run_in_executor(None, lambda: startup_routine(parameters=args, player=self.ui))
                result = r or "Done."
    ...
    async def _send_startup_briefing(self) -> None:
        ...
        async def _deliver_news():
            try:
                ...
                asyncio.create_task(self._run_startup_phase3(lang))
            except Exception as e:
    ...
    async def _run_startup_phase3(self, lang: str = "") -> None:
        try:
            await asyncio.sleep(1.0)
            result = await asyncio.to_thread(run_startup_phase3, player=self.ui)
            if result.startswith("Startup phase 3 complete"):
                self.ui.write_log("SYS: Briefing phase 3 (startup routine) completed.")
                if self.session:
                    lang_clause = f" Respond in {lang}." if lang else ""
                    await self.session.send_client_content(
                        turns={"parts": [{
                            "text": (
                                "[STARTUP_PHASE_3] The user's saved startup routine has been executed. "
                                "Briefly tell them phase 3 is done. Do not call tools."
                                f"{lang_clause}"
                            )
                        }]},
                        turn_complete=True,
                ...
                self.ui.write_log(f"SYS: Briefing phase 3 skipped: {result}")
        except Exception as e:
            print(f"[Briefing] Phase 3 error: {e}")
            self.ui.write_log(f"SYS: Briefing phase 3 failed: {e}")
