...
def _require_pyautogui():
    if any(word in message_text.lower() for word in call_words):
        return (
            "This tool only sends text messages. For a WhatsApp call, use the "
            "whatsapp tool with action='audio_call'."
        )
    if not _PYAUTOGUI:
...
def send_message(
    ...
    call_words   = ("call", "audio call", "voice call", "ring", "phone", "arama", "ara", "sesli")
